...
mod tests {
    ...
    async fn execute_nonzero_exit_stderr_is_captured() {
        // `ls /nonexistent_path` は存在しないパスのエラーを stderr に出力して非ゼロで終了する。
        // 一時スクリプトを使わない理由: Linux では書き込み用に開いたままのファイルを
        // exec しようとすると ETXTBSY が返るため、NamedTempFile のままでは実行できない。
        let outcome = execute(
        ...
        assert!(
            ...
            "exit_code should be Some non-zero"
        );
    ...
    // execute レベルで「空白のみ stderr → None」を検証するには、空白だけを stderr に
    // 出力するプロセスが必要だが、シェルスクリプトを一時ファイルで実行すると Linux の
    // ETXTBSY 制約に引っかかる。このブランチ（trim().is_empty() → None）は
    // execute() 内のロジックが単純なため、execute_empty_stderr_is_none テストで
    // カバーされる空のケースと合わせてコードレビューで確認済みとする。
}
